...
parser.add_argument('--lglist', default='pa-pnb',  type=str,
...
VOCAB = {}
for i in range(l):
    fn = args.embdir + 'cc.' + lglist[i] + '_small_20.vec'
...
for i in range(l):
    print(lglist[i], TRANS[i].shape, len(VOCAB[i]))
    save_matrix(args.outdir + '/W-' + languages + '-' + lglist[i], TRANS[i])
    save_vectors(args.outdir + '/LL-' + languages + '-' + lglist[i], EMB[i].dot(TRANS[i]), VOCAB[i])
